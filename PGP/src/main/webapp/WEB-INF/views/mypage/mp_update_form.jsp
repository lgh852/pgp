	...
	<form class="needs-validation" 
		...
		<div class="mb-3">
			...
			<input type="text" class="form-control"
				id="userId" value="${member.member_id}" name="member_id" readonly="readonly">
		</div>
		
		<c:if test="${member.member_pw==naver}">
		<div class="mb-3">
		...
		<div class="row">
			...
			<div class="col-md-6 mb-3">
				<label for="userPw2">변경하실 비밀번호 확인 <span id="userPwck2"></span></label> <input
					type="text" class="form-control" name="userPw2" id="changePw2"
					>
		...
		<input type="hidden" value="${year}" id="years">
			<input type="hidden" value="${month}" id="months">
				<input type="hidden" value="${day}" id="days">
		<div class="row">
			<div class="col-4">
...
$(document).ready(function(){
  ...
  $('#changemember').submit(function() {
		...
		alert('현재의값'+pw)
		alert('입력한 값'+userPw)
		if(changepw1.trim().length>1&&changepw2.trim().length>1){
			//변경할 비밀번호에 값이 존재할 경우 
				...
				if(pw==userPw){
					...
					if(changepw1==changepw2&&member_pw.trim().length>8){
	...
	//변경 비밀번호 체크
	$('#member_passpword').focusout(function() {
		...
		}else if(member_pw.trim().length>0&&member_pw.trim().length<8){
			// 값이 있음 
			alert(2)
